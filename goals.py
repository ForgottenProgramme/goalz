    ...
    with file.open("r") as f:
        try:
            ...
            if os.stat(file).st_size == 0:
                ...
                print(
                    f"""The following error was encountered while trying to read the file: {e}\nIf you're unable
        ...
        if check_file_not_empty(file):
            with file.open("r") as f:
                ...
                if content.get(goal_name) is not None:
                    print(
                        f"Goal '{goal_name}' already present in goal set. Update the goal instead."
                    )
                    return
                else:
                    content[goal_name] = 0
        ...
        if check_file_not_empty(file):
            ...
            if content.get(goal_name) is not None:
                if content[goal_name] < 99:
                    content[goal_name] += 1
                    print(
                        f"Progress updated for goal '{goal_name}'.\nGoal {content[goal_name]}% completed.\n"
                    )
                else:
                    print(
                        f"Yay! You just completed your 100 days goal: {goal_name}! 🎉\nMoving this goal to `completed.json` file.\n"
                    )
                ...
                print(
                    "Goal not present in goals list. Type `add <goal>` to add a new goal.\n"
                )
        else:
        ...
        if not check_file_not_empty(GOAL_FILE):
            ...
            with progress:
                for k, v in content.items():
                    progress.add_task("goal", completed=int(v), total=100, goal_name=k)
